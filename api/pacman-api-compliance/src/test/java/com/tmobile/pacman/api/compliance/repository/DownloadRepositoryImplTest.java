...
public class DownloadRepositoryImplTest {
    ...
    public void getFiltersFromDbTest() throws Exception{
        ...
        filter.put("serviceId", "1");
        filter.put("serviceName", "policyViolation");
        filter.put("serviceEndpoint", "serviceEndpoint");
        filters.add(filter);
        when(
                rdsepository.getDataFromPacman(anyString())).thenReturn(filters);
      List<Map<String,Object>>filtersfromDb=  downloadRepositoryImpl.getFiltersFromDb(1);
